...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // --- FIX: A much stricter and more reliable prompt for the first step ---
        const summaryPrompt = `
        ...
        if (!summaryResponse.ok) {
             ...
             throw new Error(`Failed to generate a search query: ${errorBody.error?.message}`);
        }
        ...
        // --- STEP 3: Second AI call to filter the search results using the SUMMARY, not the full essay ---
        ...
        if (!finalResponse.ok) {
            ...
            throw new Error(`The AI failed to filter the search results: ${errorBody.error?.message}`);
        }
