  ...
  try {
    ...
    const results = await Promise.all(urls.slice(0, 10).map(async (url) => {
      try {
        ...
        const timeoutId = setTimeout(() => controller.abort(), 10000); // 10s for slow gov sites
        ...
        // FIX: WIKIPEDIA CLEANUP
        if (url.includes('wikipedia.org')) {
            author = ""; // Force empty so citation uses Site Name
        }
