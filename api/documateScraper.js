  ...
  try {
    ...
    const results = await Promise.all(urls.slice(0, 10).map(async (url) => {
      try {
        ...
        // MIT Sloan sometimes uses 'authors'
        $('meta[name="authors"]').each((i, el) => {
            const val = $(el).attr('content');
            if (val && !authors.includes(val)) authors.push(val);
        });
        ...
        // Remove ScienceDirect garbage BEFORE extracting text
        $('.author-links, .overlay-panel, .show-more-btn').remove();
        ...
        if (byMatch) {
            ...
            const badStarts = ["The", "Contrast", "Comparison", "Definition", "Click", "Subscribe", "Research", "Capacity"]; 
            if (!badStarts.some(b => textAuthor.startsWith(b)) && textAuthor.length < 60) {
                author = textAuthor;
            }
        }

        // Strategy B: "Name, Name, and Name" (MIT Sloan style - no "By")
        // Looks for a pattern of capitalized names separated by commas/and at the START of the text
        if (!author) {
            const listRegex = /^([A-Z][a-z.-]+\s+[A-Z][a-z.-]+(?:,\s+[A-Z][a-z.-]+\s+[A-Z][a-z.-]+)*(?:,?\s+and\s+[A-Z][a-z.-]+\s+[A-Z][a-z.-]+))/;
            // Check the first 300 chars for this pattern (often appears right after title)
            const snippet = bodyText.substring(0, 300);
            const listMatch = snippet.match(listRegex);
            if (listMatch) {
                const textAuthor = listMatch[1].trim();
                // Filter out false positives (sentences starting with names)
                if (!textAuthor.includes("The ") && !textAuthor.includes("This ") && textAuthor.length < 100) {
                    author = textAuthor;
                }
            }
        }
